...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- wled_control: Control WLED smart LED lights (on/off/color/effect/brightness/preset). Use for ANY light request.\n"
        "- http_get: Make an HTTP GET request to any URL. Use for local network APIs other than WLED.\n"
        ...
        "IMPORTANT: Conversation history only saves text, not tool calls. Past assistant messages\n"
        "prefixed with [tools:...] show which tools were called that turn. If a prior response\n"
        "lacks this prefix, the tool may not have been called — do not assume it was.\n"
        "For device control (lights, OTA, HTTP), ALWAYS call the tool — never just describe the action.\n\n"
        "## Memory\n"
